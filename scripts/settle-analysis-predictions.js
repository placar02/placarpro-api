...
if (!secret) {
  console.error('Defina DAILY_PICK_PUBLISH_SECRET no .env antes de liquidar previsoes.');
  process.exit(1);
}

async function main() {
  const response = await fetch(`${baseUrl}/api/internal/analysis-predictions/settle`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'x-daily-pick-secret': secret,
    },
    body: JSON.stringify({ limit: Number(process.env.ANALYSIS_SETTLEMENT_BATCH_SIZE || 50) }),
  });
